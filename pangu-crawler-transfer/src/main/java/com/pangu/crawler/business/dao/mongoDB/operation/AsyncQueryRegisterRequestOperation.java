...
public class AsyncQueryRegisterRequestOperation {
	...
	public List<AsyncQueryRegisterRequestEntity> getByStateAndLoginMethod(int state,ArrayList<String> login_method) {
	...
	public List<AsyncQueryRegisterRequestEntity> findTimeouts(int num,ArrayList<String> login_method,long time){
		Criteria cxyj=Criteria.where("state").is(num).and("servername").is(servername).and("create_time").lte(time).and("login_method").in(login_method);
		return mongoTemplate.find(Query.query(cxyj), AsyncQueryRegisterRequestEntity.class);
	}
